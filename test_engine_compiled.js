...
var offsetsA = {};
for (var x = -20; x <= 20; x++) {
    offsetsA["".concat(x, ",").concat(TEST_WORLD_Z)] = 16;
    offsetsA["".concat(x, ",").concat(TEST_WORLD_Z + 1)] = 16;
}
engineA.updateTerrain([], [], offsetsA);
var injectZ = TEST_WORLD_Z - half + 10;
...
for (var frame = 0; frame < 120; frame++) {
    var rX = Math.floor(getRiverCenter(injectZ));
    for (var x = rX - 4; x <= rX + 4; x++) {
        var idx = (x + half) + 10 * size;
        engineA.W[idx] += 5.0;
    }
    for (var j = 0; j < 8; j++) {
        engineA.simulate();
...
var zIdx = half; // Since TEST_WORLD_Z == originZ, it's exactly at index z = size/2
for (var x = -40; x <= 40; x += 5) {
    var idx = (x + half) + zIdx * size;
    var w = engineA.W[idx];
    var diff = engineA.T[idx] - engineA.T_base[idx];
    console.log("x=".concat(x, " | Terrain Diff: ").concat(diff.toFixed(1), " | Water: ").concat(w.toFixed(2)));
}
