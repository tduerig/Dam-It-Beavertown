...
const RIVER_WIDTH = 8;
const RIVER_DEPTH = 3;
const SLOPE = 0.1; // Downhill towards +Z

function getRiverCenter(z: number): number {
  return noise2D(z * 0.01, 0) * 15; // Meandering river
}

function getBaseTerrainHeight(x: number, z: number): number {
...
const _treeCache: Record<string, any[]> = {};

function generateTreesForChunk(chunkX: number, chunkZ: number) {
...
const WATER_SIZE = Platform.OS === 'web' ? 160 : 80;
...
class WaterEngine {
  ...
  updateTerrain(blocks: PlacedBlock[], draggableLogs: DraggableLog[] = [], customOffsets: Record<string, number> | null = null) {
    // Retrieve the offsets from the state once per tick
    const offsets = customOffsets || useGameStore.getState().terrainOffsets;
...
const waterEngine = new WaterEngine();
...
for(let x=0; x<80; x++){
  for(let z=0; z<80; z++){
    engineA.T_base[x + z*80] = getBaseTerrainHeight(x - 40, TEST_WORLD_Z - 40 + z);
  }
}
...
for(let i=0; i<80*80; i++) {
   const diff = engineA.T[i] - engineA.T_base[i];
   if (diff > maxDiff) maxDiff = diff;
   if (diff > 0.5) foundDam = true;
}
console.log("Dam applied?", foundDam, "Max diff:", maxDiff);
